...
//create Send to all addresses present in conf file
//-----------------------------------------------------------------
  function createAll(){
    ...
    function buildEmaillistArray(){

      if (conf.baseOptions !== ''){
        allEmails[allEmails.length] = conf.baseOptions.to;
      }
      if (conf.yahooOptions.to !== ''){
        allEmails[allEmails.length] = conf.yahooOptions.to;
      }
      if (conf.outlookOptions.to !== ''){
        allEmails[allEmails.length] = conf.outlookOptions.to;
      }
      if (conf.gmailOptions.to !== ''){
        allEmails[allEmails.length] = conf.gmailOptions.to;
      }
      if (conf.extOptions.to !== ''){
        allEmails[allEmails.length] = conf.extOptions.to;
      }
      if (conf.litmusOpt.testEmail !== ''){
        allEmails[allEmails.length] = conf.litmusOpt.testEmail;
      }
    }
    ...
    function allMsgOpt (){
      this.from = conf.from;
      this.to = allList();
      this.subject = 'test email from etest clt';
      this.text = 'Test email from etest CLT';
      this.html = html;
    }
  ...
  function showEmails (){
    ...
    if (conf.extOptions.to !== " "){
      console.log('in gmail option: ' + conf.extOptions.to);
    }
    if (conf.litmusOpt.testEmail !== " "){

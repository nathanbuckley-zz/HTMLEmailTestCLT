...
function convert(){
  ...
  function creatAll(){
    ...
    if (conf.baseOptions !== ''){
      allEmails[0] = conf.baseOptions.to;
    }
    if (conf.yahooOptions.to !== " "){
      allEmails[1] = conf.yahooOptions.to;
    }
    if (conf.outlookOptions.to !== " "){
      allEmails[2] = conf.outlookOptions.to;
    }
    if (conf.gmailOptions.to !== " "){
      allEmails[3] = conf.gmailOptions.to;
    }
    if (conf.extOptions.to !== " "){
      allEmails[4] = conf.extOptions.to;
    }
    if (conf.litmusOpt.testEmail !== " "){
      allEmails[5] = conf.litmusOpt.testEmail;
    }
  }

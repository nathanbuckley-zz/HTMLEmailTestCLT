...
function convert(){
  ...
  var conf = JSON.parse(fs.readFileSync('node_modules/etest/lib/conf.json'));
  ...
  var msgOpt = function msgOpt (c){
    this.from = conf.from;
    this.to = conf.c.to;
    this.subject = conf.c.subject;
  ...
  //Outlook options
    var mailOpOutlook = {
      ...
      text: 'Test email. From email test CLT.', // plaintext body
      html: html
    };
  ...
  //Gmail options
    ...
    //another inbox options
      var extMail = {
        from: conf.from, // from address
        to: conf.extOptions.to, // recipient
        subject: conf.extOptions.subject, // Subject line
        text: 'Test email. From email test CLT.', // plaintext body
        html: html
      };
